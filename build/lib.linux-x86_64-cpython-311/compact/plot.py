...
def plot_multipoles_n_residuals(r, truths, preds, errs=None, label='Predicted', eps=1e-8):
    ...
    for i, ax in enumerate(axes[1]):
        if errs is not None:
            ax.plot(r, (preds[i] - truths[i])/errs[i])
        else:
            ax.plot(r, (preds[i] - truths[i])/(np.abs(truths[i])+eps))
        ax.axhline(0, color='k', ls=':')
