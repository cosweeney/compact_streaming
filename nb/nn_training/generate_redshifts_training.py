...
CompactStreamingModel across five redshifts, n_s held fixed (per the earlier
decision to defer varying it -- theta stays the original 6-vector
[b1, b2, omega_b, omega_cdm, h, As]).

z=1.317 and z=1.491 sit outside the calibrated compact-model grid
(0.3-1.1) and rely on _compact_params' cubic-spline-interp /
linear-tail-extrapolation -- already implemented in predictions.py, no
model-side changes needed here.

Required AP s-padding per redshift was derived from real CAMB background
calculations (H(z), comoving distance) over the (omega_b, omega_cdm, h)
prior box -- see AP_PADDING below. This assumes Config.r has already been
widened to cover the z=1.491 case (needs up to 171.44); the check below
will raise loudly and immediately if not.
"""
...
# required (s_min, s_max) in h^-1 Mpc, from CAMB background calc over the
# (omega_b, omega_cdm, h) prior box corners -- see the earlier AP-padding
# derivation. z=1.491 is the binding constraint on the upper end.
AP_PADDING = {
    0.510: (17.78, 148.91),
    0.706: (17.24, 154.95),
    0.919: (16.80, 160.65),
...
ELLS = (0, 2, 4)
BASE_DIR = '/spiffball/cosweeney/data/training/compact_multipole_training_set/'
NICE = 10
...
# _compact_params warns on every out-of-range call (z=1.317/1.491, every
# theta) -- that's tens of thousands of identical warnings during generation.
# Silence the repeat; the one-time acknowledgement is this docstring/review.
...
def generate_set(name, n_samples, seed, z, cfg, out_dir):
    ...
    print(f"\n{'='*70}\nz={z}  '{name}': {n_samples} samples (seed={seed})\n{'='*70}")
