#Define Functions (credit to E.M. Salazar, see https://github.com/edgarmsalazar/dynhalo/)
...
def load_galaxies(
    ...
    pos, vel, gid, gupid, row = ([[] for _ in range(len(adj_sub_box_ids))]
                          for _ in range(5))

    # Load all adjacent boxes
    for i, sub_box in enumerate(adj_sub_box_ids):
        pos[i], vel[i], gid[i], gupid[i], row[i] = _load_sub_box(
            sub_box, path, name='gal')
    # Concatenate into a single array
    pos = np.concatenate(pos)
    vel = np.concatenate(vel)
    gid = np.concatenate(gid)
    gupid = np.concatenate(gupid)
    ...
    return pos[mask], vel[mask], gid[mask], gupid[mask], row[mask]

@timer
def select_gals_around_haloes(
    n_seeds: int,
    r_max: float,
    boxsize: float,
    subsize: float,
    ...
    """Locates for the largest `v_max` seeds and searches for all the galaxies
    ...
    # Create empty lists (containers) to save the data from file for each ID
    r, R, rlos, vpeclos, vphyslos, vr, vt, M_vir, gupid, hsbid = ([] for _ in range(10))
    ...
    for sub_box_id in tqdm(unique_sub_box_ids, desc='Processing sub-box',
        ...
        pos, vel, _, upid, _ = load_galaxies(sub_box_id, boxsize, subsize, path)
        ...
        #for i in range(mask_seeds_in_sub_box.sum()):
            ...
            m_vir = mvir_seed[mask_seeds_in_sub_box][i]*np.ones_like(z_sep)
            g_upid = upid[mask_close]
            h_sbid = sub_box_id*np.ones_like(z_sep)
            ...
            # select gals within R, v_los cut
            R_max = r_max # h^-1 Mpc
            vlosmax = 6_000 #4_000 #km/s

            # Compute the radial separations
            ...
            cutmask = Rmask #* vlosmask  # uncommenting for no vlos cut
            ...
            M_vir.append(m_vir[cutmask])
            gupid.append(g_upid[cutmask])
            hsbid.append(h_sbid[cutmask])
    ...
    M_vir = np.concatenate(M_vir)
    gupid = np.concatenate(gupid)
    hsbid = np.concatenate(hsbid)

    return r, R, rlos, vpeclos, vphyslos, vr, vt, M_vir, gupid, hsbid 
...
def load_particles(
    ...
    # Load all adjacent boxes
    for i, sub_box in enumerate(adj_sub_box_ids):
        pos[i], vel[i], pid[i], _ , row[i] = _load_sub_box(
...
@timer
def select_particles_around_haloes(
    n_seeds: int,
    r_max: float,
    boxsize: float,
    subsize: float,
    ...
    # Create empty lists (containers) to save the data from file for each ID
    r, vr, vt, hsbid = ([] for _ in range(4))
    # Iterate over sub-box IDs
    # NOTE: Could parallelise this but it is not super slow
    for sub_box_id in tqdm(unique_sub_box_ids, desc='Processing sub-box',
...
@timer
def select_halos_around_haloes(
    n_seeds: int,
    r_max: float,
    boxsize: float,
    subsize: float,
    ...
    # Create empty lists (containers) to save the data from file for each ID
    r, R, rlos, vpeclos, vphyslos, vr, vt, hupid, hsbid = ([] for _ in range(9))
    # Iterate over sub-box IDs
    # NOTE: Could parallelise this but it is not super slow
    for sub_box_id in tqdm(unique_sub_box_ids, desc='Processing sub-box',
                        colour='blue', ncols=100):
        pos, vel, _, upid, _ = load_halos(sub_box_id, boxsize, subsize, path)
        ...
        for i in range(mask_seeds_in_sub_box.sum()):
            ...
            rel_vel = vel[mask_close] - vel_seed[mask_seeds_in_sub_box][i]
            z_sep = z_sep[mask_close]
            h_sbid = sub_box_id*np.ones_like(z_sep) # center of halo-halo pair' upid; "host" halo
            h_upid = upid[mask_close]
            ...
            r.append(rp[Rmask])
            R.append(Rp[Rmask])
            rlos.append(rlosp[Rmask])
            vpeclos.append(vpeclosp[Rmask])
            vphyslos.append(vphyslosp[Rmask])
            vr.append(vrp[Rmask])
            vt.append(vtp[Rmask])
            hupid.append(h_upid[Rmask])
            hsbid.append(h_sbid[Rmask])
    ...
    return r, R, rlos, vpeclos, vphyslos, vr, vt, hupid, hsbid
...
@timer
def select_halos_around_haloes_parallel(
    n_seeds: int,
    r_max: float,
    boxsize: float,
    subsize: float,
    ...
    def _process_sub_box(sub_box_id):
        ...
        r_sb, R_sb, rlos_sb, vpeclos_sb, vphyslos_sb, vr_sb, vt_sb, hupid_sb, hsbid_sb = (
            [] for _ in range(9)
        )
        ...
        for i in range(mask_seeds_in_sub_box.sum()):
            ...
            vphyslos_sb.append(vphyslosp[Rmask])
            vr_sb.append(vrp[Rmask])
            vt_sb.append(vtp[Rmask])
            hupid_sb.append(h_upid[Rmask])
            hsbid_sb.append(h_sbid[Rmask])
        ...
        return (
            ...
            np.concatenate(vphyslos_sb),
            np.concatenate(vr_sb),
            np.concatenate(vt_sb),
            np.concatenate(hupid_sb),
            np.concatenate(hsbid_sb),
        )
    ...
    r, R, rlos, vpeclos, vphyslos, vr, vt, hupid, hsbid = (
...
def select_gals_around_gals(
    ...
    """Locates for the largest `v_max` gals and searches for all the gals
    around them up to a distance `r_max`.
    ...
    with h5.File(file_seeds, 'r') as hdf:
        ...
        grvir = hdf['r'][()][mass_mask]

    vmax = vmax[order][:]
    gid = gid[order][:]
    pos_seed = pos_seed[order][:]
    vel_seed = vel_seed[order][:]
    rvir_seed = grvir[order][:]
    ...
    # Create empty lists (containers) to save the data from file for each ID
    r, R, rlos, vpeclos, vphyslos, vr, vt, gupid, gsbid = ([] for _ in range(9))
    # Iterate over sub-box IDs
    # NOTE: Could parallelise this but it is not super slow
    for sub_box_id in tqdm(unique_sub_box_ids, desc='Processing sub-box',
                        colour='blue', ncols=100):
        ...
        for i in range(mask_seeds_in_sub_box.sum()):
            ...
            rel_vel = vel[mask_close] - vel_seed[mask_seeds_in_sub_box][i]
            z_sep = z_sep[mask_close]
            g_sbid = sub_box_id*np.ones_like(z_sep) # center of gal-gal pair's upid; "host" gal
            g_upid = upid[mask_close]
            ...
            vr.append(vrp[Rmask])
            vt.append(vtp[Rmask])
            gupid.append(g_upid[Rmask])
            gsbid.append(g_sbid[Rmask])
    ...
    vt = np.concatenate(vt)
    gupid = np.concatenate(gupid)
    gsbid = np.concatenate(gsbid)

    return r, R, rlos, vpeclos, vphyslos, vr, vt, gupid, gsbid


from joblib import Parallel, delayed


def _process_sub_box(
    sub_box_id: int,
    seed_sub_box_id: np.ndarray,
    pos_seed: np.ndarray,
    vel_seed: np.ndarray,
    r_max: float,
    boxsize: float,
    ...
    """Processes a single sub-box: loads its galaxies and searches for
    neighbors around every seed located in this sub-box. This is the body
    of the original `select_gals_around_gals` outer loop, extracted so it
    can be dispatched to a worker process.

    Returns
    -------
    Tuple[np.ndarray]
        Radial distance, projected distance, line-of-sight separation,
        peculiar/physical LOS velocity, radial/tangential velocity, upid,
        and sub-box id, concatenated over all seeds in this sub-box.
    """
    pos, vel, _, upid, _ = load_galaxies(sub_box_id, boxsize, subsize, path)
    ...
    mask_seeds_in_sub_box = seed_sub_box_id == sub_box_id
    seeds_pos = pos_seed[mask_seeds_in_sub_box]
    seeds_vel = vel_seed[mask_seeds_in_sub_box]

    for i in range(mask_seeds_in_sub_box.sum()):
        rel_pos = relative_coordinates(seeds_pos[i] % boxsize, pos, boxsize)
        z_sep = pos[:, 2] % boxsize - seeds_pos[i][2] % boxsize
        ...
        rel_pos = rel_pos[mask_close]
        rel_vel = vel[mask_close] - seeds_vel[i]
        z_sep = z_sep[mask_close]
        g_sbid = sub_box_id * np.ones_like(z_sep)
        g_upid = upid[mask_close]
    ...
    return (
        ...
        np.concatenate(R),
        np.concatenate(rlos),
        np.concatenate(vpeclos),
        np.concatenate(vphyslos),
        np.concatenate(vr),
        np.concatenate(vt),
        np.concatenate(gupid),
        np.concatenate(gsbid),
    )


@timer
def select_gals_around_gals_para(
    n_seeds: int,
    r_max: float,
    boxsize: float,
    subsize: float,
    ...
    """Locates for the largest `v_max` gals and searches for all the gals
    around them up to a distance `r_max`. Parallelized over sub-boxes.
    ...
    path : str
        Path to the sub-boxes
    mass_mask : np.ndarray
        Mask selecting which entries in `file_seeds` to use as seeds
    n_jobs : int, optional
        Number of worker processes for joblib.Parallel, by default -1 (all cores)
    ...
    # Process each sub-box in parallel; each is fully independent (own
    # load_galaxies I/O + seed search against the shared, read-only seed arrays)
    results = Parallel(n_jobs=n_jobs)(
        delayed(_process_sub_box)(
            sub_box_id, seed_sub_box_id, pos_seed, vel_seed, r_max, boxsize, subsize, path
        )
        for sub_box_id in tqdm(unique_sub_box_ids, desc='Processing sub-box',
                                colour='blue', ncols=100)
    )

    # Concatenate across sub-boxes
    r, R, rlos, vpeclos, vphyslos, vr, vt, gupid, gsbid = (
        np.concatenate(arrs) for arrs in zip(*results)
    )
